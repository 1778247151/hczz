...
import com.crm.common.BaseResultVo1;
...
public class AnnotationHandlerMethodExceptionResolver extends ExceptionHandlerExceptionResolver {  
    ...
    protected ModelAndView doResolveHandlerMethodException(HttpServletRequest request, HttpServletResponse response, HandlerMethod handlerMethod, Exception exception) {  
        ...
        if (responseBodyAnn != null) {  
            try {  
                ...
                if (returnValue == null) {  
                	//异常信息类
                    BaseResultVo1 res = new BaseResultVo1();  
                    res.setCode(ResponseCode.SYS_ERROR);  
                    res.setMsg("系统异常：" + exception.getMessage());  
    ...
    @SuppressWarnings({ "unchecked", "rawtypes" })  
    private ModelAndView handleResponseError(BaseResultVo1 returnValue, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {  
        HttpInputMessage inputMessage = new ServletServerHttpRequest(request);  
        List<MediaType> acceptedMediaTypes = inputMessage.getHeaders().getAccept();  
        if (acceptedMediaTypes.isEmpty()) {  
            acceptedMediaTypes = Collections.singletonList(MediaType.ALL);  
