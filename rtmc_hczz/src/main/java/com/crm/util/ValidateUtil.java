...
public class ValidateUtil {
	...
	/**
	 * 
	 * isAvailableCodeStr:只能输入数字、字母或下划线
	 *
	 * @author   shengjinpeng
	 * @date     2016年8月17日
	 *
	 ...
	 * @return
	 */
	public static boolean isAvailableCodeStr(String str){
		if(str == null){
			return false;
		}
		return validate(str, "^[a-zA-Z0-9_]+$");
	}
...
//		System.out.println(validate("Content-Type: text/html; charset=GBK kjlfdbsfdsfldsl1231^%$$% fdsdskfjdlskfjdlskfdlsjfldsjfls", "charset=(\\w+)"));
	...
	public static boolean validateStr(Object[] validateStrs,Object[] validateTypes,HttpServletResponse response){
		if(validateStrs.length!=validateTypes.length){
			logger.info("需要验证的参数和验证规则不匹配");
			return false;
		}
		else{
			//
			for (int i = 0; i < validateStrs.length; i++) {
				String str=validateStrs[i].toString();
