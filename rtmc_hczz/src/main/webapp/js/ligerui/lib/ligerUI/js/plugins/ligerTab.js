...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        toggleSwitch: function (btn)
        {
            ...
            if (btn == null) return;
            var windowsswitch = $("<div class='l-tab-windowsswitch' style='width:140px;'></div>").appendTo('body');
            ...
            if(tabItems.size()>2){
            	 var jlink = $("<a class=\"close_tab\" href='javascript:void(0)'>关闭所有选项卡</a>");
                // jlink.text($(item).find("a").text());
                
                 windowsswitch.append(jlink);
                // windowsswitch.remove();
            }
            windowsswitch.find("a.close_tab").bind("click", function (e)
             {
            	closeAllTab();
            	$("body > .l-tab-windowsswitch").remove();
            });
            ...
            windowsswitch.css({
                ...
                left: $(btn).offset().left - windowsswitch.width()+138 
            });
