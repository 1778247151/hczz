...
$(function (){	
	...
	//创建数据列表
	dataUrl = baseUrl + "stockCheck/getStockCheckList.do";
...
var _flag = $.fsh.noteForNotSelected("请选中数据后，再操作！");
if(_flag){
		var url="/manager/gw_stock_check/gw_stock_check_consult.html?id="+_flag.id+'&code='+_flag.code+'&temp=1'+'&status='+_flag.status;
...
function getStockCheckCountByCondition(){
	...
	dataUrl = baseUrl+"stockCheck/getStockCheckList.do?code="+code+'&date='+date+'&status='+status+'&storeCode='+storeCode
