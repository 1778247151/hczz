...
function addVipInfo() {
	var url="/manager/gw_members/gw_vipinfo_add.html";
	parent.f_addTab("addMember","会员开卡",url);
	//$.ligerDialog.open({ width:640, height: 500, url: url, isResize: false, title: "添加部门"});
}

function delVipInfo(){
	var _flag = $.fsh.noteForNotSelected("请选中数据后，再操作！");
	var vipCode=_flag.vipCode;
	
	if(_flag) {
		var url = baseUrl + "members/delMembers.do?id=" +_flag.id+"&vipCode="+vipCode;
...
function updateVipInfo(){
	var _flag = $.fsh.noteForNotSelected("请选中数据后，再操作！");
	if(_flag){
		var url="/manager/gw_members/gw_vipinfo_edit.html?id="+_flag.id;
		parent.f_addTab("editMember","更改会员信息",url);
		//$.ligerDialog.open({ width:640, height: 500, url: url, isResize: false, title: "修改部门"});
	}
}
...
function batchUpdateCredit(){
	...
	if(ids==null || ids.length==0){
		...
		$.ligerDialog.open({ width:640, height: 500, url: url, isResize: false, title: "批量修改积分"});
	}
}
