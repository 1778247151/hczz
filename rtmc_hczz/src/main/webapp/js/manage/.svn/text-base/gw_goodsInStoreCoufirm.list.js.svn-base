...
$(function (){	
	...
	//创建数据列表
	dataUrl = baseUrl + "goodsInfoStore/getGoodsInStoreCoufirmByPageList.do";
...
var _flag = $.fsh.noteForNotSelected("请选中数据后，再操作！");
if(_flag){
		var url="/manager/gw_goods_in_store/gw_goods_in_store_consult.html?id="+_flag.id+'&code='+_flag.code+'&status='+_flag.status
		parent.f_addTab("goodsInStoreConfirm","确认物品入库信息",url);	
}
...
function getGoodsInStoreByCondition(){
	...
	dataUrl = baseUrl+"goodsInfoStore/getGoodsInStoreCoufirmByPageList.do?code="+code+'&inTime='+inTime+'&status='+status
