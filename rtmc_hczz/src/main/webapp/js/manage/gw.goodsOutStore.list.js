...
$(function (){	
	...
	//创建数据列表
	dataUrl = baseUrl + "goodsOutStore/getGoodsOutStoreByPageList.do";
...
//修改物品出库信息
function editGoodsOutStore(){
var _flag = $.fsh.noteForNotSelected("请选中数据后，再操作！");
if(_flag){
	if(_flag.status == 1){
		$.ligerDialog.warn("已确认的物品出库单不能修改")
	}else{
		var url="/manager/gw_goods_out_store/gw_goods_out_store_edit.html?id="+_flag.id+'&code='+_flag.code
		parent.f_addTab("goodsOutStoreEdit","修改物品出库信息",url);
	}
...
//添加物品出库信息
function addGoodsOutStore(){
	var url="/manager/gw_goods_out_store/gw_goods_out_store_add.html";
	parent.f_addTab("goodsOutStoreAdd","添加物品出库信息",url);
} 

//删除物品出库单信息
function deleteGoodsOutStore(){
var _flag = $.fsh.noteForNotSelected("请选中数据后，再操作！");
	if(_flag) {
		if(_flag.status == 1){
			$.ligerDialog.warn("已确认的物品出库单不能删除")
		}else{
			var url = baseUrl + "goodsOutStore/deleteGoodsOutStoreById.do?id=" +_flag.id+'&goodsOutStoreCode='+_flag.code;
