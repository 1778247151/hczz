...
(function($){
	...
	window.fsh = $.fsh = {
		...
		*/
		...
		@param multSelect 是否支持多选 默认为false
		@param _width  宽度
		@param _height 高度
		@return obj
		*/
		...
		*/
		...
		isSelected2: function( name ){
			if(name){
				return 	grid2.rowdata(name);
			}else{
				return grid2.getSelectedRow();
			}
		},
		/**
		* 获取多选的值
		...
		*/
		...
		noteForNotSelected2: function( str ){
			var _flag = $.fsh.isSelected2();
			if(!_flag){
				$.ligerDialog.warn( str );
				return;
			}
		...
		*/
		...
		disableSelectedRow: function(url, grid, msg) {
			var flag = true,
				re_info;
		...
		CoufrimSelectedRow: function(url, grid, msg) {
			var flag = true,
				re_info;
		...
		startThisActivitySelectedRow: function(url, grid, msg) {
			var flag = true,
				re_info;
			$.ligerDialog.confirm("确定要开始此活动规则？", function( r ){
				if(r){
					re_info = $.fsh.getAjaxData( url );
					if(re_info.status.code=="10000"){
						grid.reload();
		...
		stopThisActivity: function(url, grid, msg) {
			var flag = true,
				re_info;
			$.ligerDialog.confirm("确定要强制终止此活动？", function( r ){
				if(r){
					re_info = $.fsh.getAjaxData( url );
					if(re_info.status.code=="10000"){
						grid.reload();
		...
		enbleThisActivity: function(url, grid, msg) {
			var flag = true,
				re_info;
			$.ligerDialog.confirm("确定要启用该活动？", function( r ){
				if(r){
					re_info = $.fsh.getAjaxData( url );
					if(re_info.status.code=="10000"){
						grid.reload();
		...
		startEnter: function(url, grid, msg) {
			var flag = true,
				re_info;
			$.ligerDialog.confirm("确定要开始报名此活动吗？", function( r ){
				if(r){
					re_info = $.fsh.getAjaxData( url );
					if(re_info.status.code=="10000"){
						grid.reload();
		...
		stopEnter: function(url, grid, msg) {
			var flag = true,
				re_info;
			$.ligerDialog.confirm("确定要结束报名此活动吗？", function( r ){
				if(r){
					re_info = $.fsh.getAjaxData( url );
					if(re_info.status.code=="10000"){
						grid.reload();
		...
		startActivity: function(url, grid, msg) {
			var flag = true,
				re_info;
			$.ligerDialog.confirm("确定要开始此活动吗？", function( r ){
				if(r){
					re_info = $.fsh.getAjaxData( url );
					if(re_info.status.code=="10000"){
						grid.reload();
		...
		stopActivity: function(url, grid, msg) {
			var flag = true,
				re_info;
			$.ligerDialog.confirm("确定要结束此活动吗？", function( r ){
				if(r){
					re_info = $.fsh.getAjaxData( url );
					if(re_info.status.code=="10000"){
						grid.reload();
